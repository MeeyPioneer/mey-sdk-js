/**
 * These tests need a Ledger Nano S with the MeyCoin Ledger App connected on USB.
 */
...
import { hash, verifyTxSignature, verifySignature, publicKeyFromAddress, hashTransaction } from '@mey-sdk-js/crypto';
import MeyCoinClient, { Tx } from '@mey-sdk-js/client';
import TransportNodeHid from '@ledgerhq/hw-transport-node-hid';
import Transport from '@ledgerhq/hw-transport';
import MeyCoinLedgerApp from '../src';
import { Buffer } from 'buffer';

let transport: Promise<Transport>;

async function getApp(): Promise<MeyCoinLedgerApp> {
    if (!transport) {
        transport = TransportNodeHid.create(3000, 1500);
    }
    return transport.then(transport => new MeyCoinLedgerApp(transport));
}
...
async function signAndVerifyTx(app: MeyCoinLedgerApp, txBody: any): Promise<boolean> {
    // eslint-disable-next-line no-console
...
describe.skip('sign and send', () => {
    ...
    it('should sign transaction and send to network', async () => {
        ...
        // ATM, mey-sdk-js/client does not depend on mey-sdk-js/crypto. Maybe hashing should be in a 'common' package?
