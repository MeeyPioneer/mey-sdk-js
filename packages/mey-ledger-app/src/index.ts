...
import { Address, } from '@mey-sdk-js/common';
// @ts-ignore
import { Tx } from '@mey-sdk-js/client';
 ...
 * at https://github.com/meycoin/mey-sdk-js/blob/develop/src/transaction.h
 */
...
export default class MeyCoinLedgerApp {
    transport: Transport;
    lastAddress?: Address;

    constructor(transport: Transport, scrambleKey = 'mey') {
    ...
    /**
     * Get version of MeyCoin Ledger app
     */
    ...
    /**
     * Get an MeyCoin address from Ledger given a BIP44 path
    ...
    async signTransaction(tx: any): Promise<SignTxResponse> {
        ...
        if (supportedTypes.indexOf(tx.type) === -1) {
            throw new Error('Transaction type not supported by MeyCoin Ledger app');
        }
