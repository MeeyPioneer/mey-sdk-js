...
import { Amount, Address } from '@mey-sdk-js/common';
import { ACCOUNT_UPDATE_INTERVAL } from '../defaults';
import { PausableTypedEventEmitter } from '../utils';
import { createIdentity, identifyFromPrivateKey } from '@mey-sdk-js/crypto';
import { Identity } from '@mey-sdk-js/crypto/dist/types/keys';
...
export default class AccountManager extends PausableTypedEventEmitter<Events> {
     ...
     * This uses the MeyCoin client to determine the nonce from the server.
     * @param account 
     */
    ...
    async prepareTransaction(account: Account, tx: Partial<TxBody>): Promise<Transaction> {
        ...
        tx.amount = new Amount(tx.amount).toUnit('gas').toString();
