...
export class Wallet extends MiddlewareConsumer {
    ...
    private config: WalletConfig = { appName: 'mey-sdk-js-wallet', appVersion: 1, instanceId: '' };
    public datastore?: Storage;
    public keystore?: Storage;
    private clients: Map<string, MeyCoinClient> = new Map();
    public defaultLimit?: number;
    public ledger?: MeyCoinLedgerApp;
    ...
    useChain(chainConfig: ChainConfig): void {
        if (typeof chainConfig.provider === 'undefined') {
            chainConfig.provider = MeyCoinClient.defaultProviderClass;
        }
    ...
    /**
     * Get MeyCoinClient for chainId.
     * If called the first time, create MeyCoinClient instance.
     * @param chainId optional, uses default chainId when undefined
     */
    getClient(chainId?: string): MeyCoinClient {
        ...
        if (!this.clients.has(chainId)) {
            ...
            client = new MeyCoinClient({}, provider);
            this.clients.set(chainId, client);
        } else {
            client = this.clients.get(chainId) as MeyCoinClient;
        }
    ...
    connectLedger(transport: any): void {
        this.ledger = new MeyCoinLedgerApp(transport);
    }
}
