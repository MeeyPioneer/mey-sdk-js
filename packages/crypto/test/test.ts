...
describe('hashTransaction()', () => {
    it('should default amount to 0 and treat falsy values as 0', async () => {
        ...
        const amounts = ['0 gas', ' ', '0', 0, '', false, null, undefined];
    ...
    it('should fail with invalid unit', async () => {
        ...
        const amounts = ['100000 mey', 'foo mey', 'one gas', '123 whatever'];
        for (const amount of amounts) {
            const tx = {
                amount,
                nonce: 1,
            ...
            await assert.isRejected(hashTransaction(tx), Error, `Can only hash amounts provided in the base unit (gas), not '${amount}'. Convert to gas or remove unit.`);
        }
    });
    it('should fail with non-numeric amounts', async () => {
        // These are all invalid and throw 'invalid numeric value' error
        const amounts = ['gas', ' gas', '  gas  '];
        for (const amount of amounts) {
            const tx = {
                amount,
                nonce: 1,
...
describe('encodeAddress()', () => {
    it('should encode an address of bytes to a string', async () => {
        assert.equal(encodeAddress(Buffer.from('mey.system')), 'mey.system');
...
describe('keystore', () => {
    ...
    it('should generate a keystore file', async () => {
        ...
        assert.equal(keystore.mey_address, address);
