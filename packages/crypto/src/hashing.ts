...
function inferType(tx: TxBody): number {
    ...
    if (`${tx.to}` === 'mey.system' || `${tx.to}` === 'mey.enterprise') {
        return 1;
    }
    return 0;
}
...
export async function hashTransaction(tx: TxBody, encoding: ByteEncoding | 'bytes' = 'base64', includeSign = true): Promise<Buffer | string> {
    ...
    if (tx.amount) {
        ...
        if (amountStr !== '') {
            // Throw error if unit is given other than gas
            const amountUnit = amountStr.match(/\s*([^0-9]+)\s*/);
            if (amountUnit && amountUnit[1] !== 'gas') {
                throw Error(`Can only hash amounts provided in the base unit (gas), not '${tx.amount}'. Convert to gas or remove unit.`);
            }
            // Strip unit
